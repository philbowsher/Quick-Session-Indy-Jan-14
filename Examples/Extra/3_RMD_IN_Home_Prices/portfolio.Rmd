...
### Visualization of IN home values built with ggplot2 {data-commentary-width=700}
...
ILzip <- house %>% filter(State == 'IN') %>% 
  gather(month, zhvi, starts_with('X')) %>% 
  filter(!is.na(zhvi)) %>% 
  filter(month == 'X2019.09')
...
plots <- ggplot(ILzip, 
  ...
  labs(x = 'Zip Code', y = 'Zillow Home Value Index', title = 'IN Home Values Oct 2019') + 
